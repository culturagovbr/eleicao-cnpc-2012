...
class GenericController extends Zend_Controller_Action
{
	...
	public function init()
	{
		...
		$this->diretorioUpload = "/var/www/cnpc.cultura.gov.br/public/anexos/";
		$this->caminhoAcessoArquivo = "http://cnpc.cultura.gov.br/public/anexos/";
...
//            $pdf->WriteHTML("PDF Gerado em ".$totaltime." segundos");
        ...
        public function setarValoresCamposViewRecurso($rsObj,$rsCadastro){
            ...
            foreach($rsObj as $chave => $campo){
                ...
                if(file_exists($urlArquivo)){
                    ...
                    $arrRetorno[$chave]['id_anexo_recurso'] = $campo->id_anexo_recurso;
                    $arrRetorno[$chave]['id_item']          = $campo->id_item;
                }
            }
        ...
        public function setarValoresCamposViewForum($rsObj,$blnForum=false){
         //xd($rsObj);
            foreach($rsObj as $campo){
                
		$cpf = $campo->vhr_login;
		$idCadastro = $campo->id_cadastro;
                ...
                if($campo->id_item == 28){ //propostas (carta programa em texto)
                    if(trim($campo->vhr_valor) != ""){
                        $this->view->propostas = $campo->vhr_valor;
                        $this->view->file_carta_programa = null;
                        $this->view->arquivo_carta_programa = null;
                        ...
                        $urlArquivo = null;
                        $nomeArquivo = null;
                        //ESTE DOCUMETNO SÓ É POSSIVEL ENVIAR POR MEIO DE RECURSO
                        $tbAnexoRecurso = new AnexoRecurso();
                        $where = array();
                        $where['ar.id_item = ?'] = 35; //CODIGO - CARTA PROGRAMA.PDF
                        $where['c.id_cadastro = ?'] = $idCadastro;
                        $rsAnexoRecurso = $tbAnexoRecurso->buscaCompleta($where)->current();
                        if(!empty($rsAnexoRecurso)){

                            $urlArquivo = $rsAnexoRecurso->vhr_caminho_anexo;
                            $nomeArquivo = explode("/",$urlArquivo);
                            $nomeArquivo = $nomeArquivo[count($nomeArquivo)-1];
                        ...
                        $this->view->file_carta_programa = (file_exists($urlArquivo)) ? $link : "";
                        $this->view->arquivo_carta_programa  = $nomeArquivo; //$nome_arquivo;
                    }
                }
                if($campo->id_item == 29){ //file_curriculo
                    //$this->view->file_curriculo = $campo->vhr_valor;
                    ...
                    //VERIFICA SE ENVIOU O ARQUIVO POR MEIO DA SOLICITACAO DE RECURSO
                    if(!file_exists($urlArquivo)){
                        
                        $tbAnexoRecurso = new AnexoRecurso();
                        $where = array();
                        $where['ar.id_item = ?'] = 29; //CODIGO - CURRICULO.PDF
                        $where['c.id_cadastro = ?'] = $idCadastro;
                        $rsAnexoRecurso = $tbAnexoRecurso->buscaCompleta($where)->current();
                        if(!empty($rsAnexoRecurso)){
                    ...
                    //xd($link);
                    $this->view->file_curriculo = (file_exists($urlArquivo)) ? $link : "";
                    //$this->view->file_curriculo = $link;
                    $this->view->arquivo_curriculo  = $nomeArquivo; //$nome_arquivo;
                    $this->view->idCadastroXItem_curriculo  = $campo->id_cadastro_item;
                    $this->view->rd_curriculo  = $campo->chr_validacao;
                    $this->view->obs_curriculo = $campo->vhr_observacao;
                }else{
                ...
                if($campo->id_item == 30){ //file_portfolio
                    ...
                    if(!file_exists($urlArquivo)){
                        ...
                        $tbAnexoRecurso = new AnexoRecurso();
                        $where = array();
                        $where['ar.id_item = ?'] = 30; //CODIGO - PORTIFOLIO.PDF
                        $where['c.id_cadastro = ?'] = $idCadastro;
                        $rsAnexoRecurso = $tbAnexoRecurso->buscaCompleta($where)->current();
                        if(!empty($rsAnexoRecurso)){
                    ...
                    $this->view->file_portfolio = (file_exists($urlArquivo)) ? $link : "";
                    $this->view->arquivo_portfolio  = $nomeArquivo;//$nome_arquivo;
                    $this->view->idCadastroXItem_portfolio  = $campo->id_cadastro_item;
                    $this->view->rd_portfolio  = $campo->chr_validacao;
                    $this->view->obs_portfolio = $campo->vhr_observacao;
                }
                if($campo->id_item == 32){ //file_carta_apoio
                    ...
                    if(!file_exists($urlArquivo)){
                        ...
                        $tbAnexoRecurso = new AnexoRecurso();
                        $where = array();
                        $where['ar.id_item = ?'] = 32; //CODIGO - CARTA APOIO.PDF
                        $where['c.id_cadastro = ?'] = $idCadastro;
                        $rsAnexoRecurso = $tbAnexoRecurso->buscaCompleta($where)->current();
                        if(!empty($rsAnexoRecurso)){
                    ...
                    $this->view->file_carta_apoio = (file_exists($urlArquivo)) ? $link : "";
                    $this->view->arquivo_carta_apoio  = $nomeArquivo; //$nome_arquivo;
                    $this->view->idCadastroXItem_carta_apoio  = $campo->id_cadastro_item;
                    $this->view->rd_carta_apoio  = $campo->chr_validacao;
                    $this->view->obs_carta_apoio = $campo->vhr_observacao;
                }
            ...
            if(!empty($rsAvalicaoInscrito)){
                ...
                if($rsAvalicaoInscrito->chr_avaliacao_eleitor == 1) //eleitor validado
                {
                    ...
                    if(!empty($rsHabilitacaoSetorialUF)){
                        ...
                        unset($_SESSION);
                        $this->_redirect("login/error/?msg=01&type=ALERT");
                        return "false";
                    }
                }else{
                    //return "false";
